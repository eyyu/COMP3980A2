...
void printData(gpsmm& gpsData) {
	...
	bool 	used; 		// DEBUG : storing vals if necessary -- to be simplified later
    ...
    else 
    {
    	...
    	if(gpsData.data()->fix.mode >1){ /* 2D or 3D */
		    ...
		    longitude =	gpsData.data()->fix.longitude;
		 	...
		 	else /*EY - 4 : IS THIS NECESSARY? */
		 	{
		 		//cout << "Not 3D Mode, cannot get altitude and climb";
		 	}
    	}
		 ...
		 for(int i = 0; i < gpsData.data()->satellites_visible; ++i )
		 {
			 	...
			 	used = gpsData.data()->skyview[i].used;
		 }
 ...
 /**
  * NOTE TO SELF:  isNorthSOuthOnLongitute and isEastWestonLatitude Functions 
  */